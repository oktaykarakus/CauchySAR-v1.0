function data = bpFastFarField(data, max_recursion_depth, oversampling_ratio, ...
...
%
% Title: Fast SAR Back-projection
%
% Version: 0.3.0
%
% Description: This function performs the fast back-projection
...
%              Downsampling filter lengths. filter_size(1) specifies
...
%
% Example:
% data = bpFast(data, max_recursion_depth, oversampling_ratio, ...
...
p.addOptional('filter_size', [41, 41], ...
              @(x) isvector(x) && length(x) == 2);
...
% set minF elements to the same value and correspondingly correct
% phdata
...
data = recursive_bp(data, max_recursion_depth, 0, ...
                    floor(log2(oversampling_ratio)), decimation_phdata);

% restore minF elements
data.minF = minF;

function data = recursive_bp(data, max_recursion_depth, recursion_depth, ...
...
%                 radial_shift = sub_data.R0 - data.R0;
                radial_shift = -(im_final_center_x * cosd(data.AntElev) .* cosd(data.AntAzim) + ...
                    im_final_center_y * cosd(data.AntElev) .* sind(data.AntAzim) + ...
                    im_final_center_z * sind(data.AntElev));
                % focus sub-image phase histories to the new scene
                % reference point by using the fourier shift property
                sub_data.phdata = data.phdata .* ...
                        exp(1i * 4 * pi * ...
                            (data.minF(1) + ...
                ...
                if recursion_depth >= max_oversampling_depth + 1
                    ...
                    % low-pass filter with over-sampling to remove
                    ...
                    sub_data.phdata = sub_data.phdata .* low_pass_filt;
                    sub_data.phdata = ...
                        sub_data.phdata((1:size(low_pass_filt, 1) / 2), :) + ...
                        sub_data.phdata((size(low_pass_filt, 1) / 2 + 1):end, :);
                    sub_data.phdata = ...
                        sub_data.phdata(:, (1:size(low_pass_filt, 2) / 2)) + ...
                        sub_data.phdata(:, (size(low_pass_filt, 2) / 2 + 1):end);

                    sub_data.phdata = 4 * fft2(sub_data.phdata);

                    sub_data.phdata = ...
                        sub_data.phdata(1:ceil(phdata_size(1) / 2), ...
                                        1:ceil(phdata_size(2) / 2));

                    sub_data.deltaF = data.deltaF * 2;
                    sub_data.Nfft = ceil(data.Nfft / 2);

                    sub_data.minF = data.minF(1:2:end);
                    sub_data.AntElev = data.AntElev(1:2:end);
                    sub_data.AntAzim = data.AntAzim(1:2:end);
...
%                     sub_data.R0 = sub_data.R0(1:2:end);
                ...
                %% RECURSIVE CALL
                sub_data = recursive_bp(sub_data, ...
                                        max_recursion_depth, ...
                                        recursion_depth, ...
                ...
                if ii == 1
                    ...
                    im_final = complex(zeros(im_final_size));
                end

                %% combine split images

                % place the sub-image into the larger image
                im_final(sub_im_final_y_range, sub_im_final_x_range) = ...
                    sub_data.im_final;
            else
                %
                % Decimation in Image
...
%                 sub_data.R0 = data.R0(sub_phdata_x_range);
                sub_data.AntElev = data.AntElev(sub_phdata_x_range);
                sub_data.AntAzim = data.AntAzim(sub_phdata_x_range);
...
%                     sub_data.R0(sub_phdata_center_x_idx);
                sub_data.dR0 = -(data.x_mat * cosd(sub_data.AntElev(sub_phdata_center_x_idx))...
                    .* cosd(sub_data.AntAzim(sub_phdata_center_x_idx)) + data.y_mat...
                    * cosd(sub_data.AntElev(sub_phdata_center_x_idx))...
                    .* sind(sub_data.AntAzim(sub_phdata_center_x_idx)) + ...
                    data.z_mat * sind(sub_data.AntElev(sub_phdata_center_x_idx)));
                if recursion_depth >= max_oversampling_depth + 1
                    %% image decimating

                    sub_data.im_final = data.im_final(1:2:end, 1:2:end);
                ...
                if recursion_depth == max_recursion_depth
                    % center sub-image spectrum by using the
                ...
                if recursion_depth >= max_oversampling_depth + 1
                    ...
                    sub_data.im_final = ...
                        fft2(sub_data.im_final .* low_pass_filt);

                    sub_data.im_final = ...
                        sub_data.im_final(1:im_final_size(1), ...
                                          1:im_final_size(2));
                end

                % center upsampled sub-image spectrum to new center by using the
                % fourier shift property
                ...
                % combine the low resolution sub-images into the
                % larger resolution image
                im_final = im_final + sub_data.im_final;
            end
        end
