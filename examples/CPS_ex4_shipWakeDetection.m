...
% wake detection in SAR images in terms of the non-convex Cauchy penalty function.
% The script below fursther detects ship wakes by using the inverse problem
% solution.
% We  assume there exist 5 ship wake structures, which are:
% 1 Turbulent wake,
% 2 arms of Narrow V wake,
% 2 arms of Kelvin wake.
% For details please refer to the references given below.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
%       ** CauchyReconsIm: Stores reconstructed SAR images for each theta
%       interval.
%
%       ** mu: CPS step size.
%
%       ** gamma: Scale parameter of Cauchy-penalty function.
%
...
%
%       ** wakeDetectedIm: Resuting image with embedded detected-validated ship wakes.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% LICENSE
% 
...
% [2] O Karakus, and A Achim. "On Solving SAR Imaging Inverse Problems Using 
%       Non-Convex Regularization with a Cauchy-based Penalty" 
%       IEEE Transactions on Geoscience and Remote Sensing, 2020.
% [3] O Karakus, P Mayo, and A Achim. "Convergence Guarantees for
...
mu = 0.01;                                                                
gamma = 2*(sqrt(mu)/2);                                                                                     
load('testImage1.mat');                                                      % Loading the test image. It returns an image within a variable "Image"
%% Inverse problem solution
CauchyReconsIm = zeros([size(Image) numRotate]);
for rotation = 1:numRotate
    ...
    x = zeros(size(radon(eye(size(Image)), theta)));
    u = zeros(size(x));
    oldX = x;
    deltaX = inf;
    maxIter = 200;
    for  k = 1:maxIter
        if deltaX(k)>1e-3
            u = x - mu * AH(A(x) - Image);
            x = CauchyProx(u, gamma, mu);
        end
        deltaX(k+1) = max(abs( x(:) - oldX(:) )) / max(abs(oldX(:)));
        oldX = x;
    end
    tempReconsIm = A(x);                                                    % Tranform into image domain.
    tempReconsIm = tempReconsIm/max(tempReconsIm(:));                                % Normalization between 0 and 1.
    CauchyReconsIm(:, :, rotation) = tempReconsIm;                                   % Storing results.
end
...
for rotation = 1:numRotate
    ...
    A  = @(x) radonT(x,theta);
    AH = @(x) radon(x,theta);
    tempReconsIm = CauchyReconsIm(:, :, rotation);
    [Rver, xp] = AH(tempReconsIm);%
...
[F, wakeDetectedIm] = confirmedHalflines(lineStats, thetaInt2, ipSolvedImage, Rver, Image);
...
figure(1)
imshowpair(Image, wakeDetectedIm, 'montage')
drawnow